...
uv run ruff check --select D --fix     # Check docstrings (Google style)
...
```powershell
uv run gepa-adk          # Run via CLI entry point
```
...
### Key Commands
```bash
# Run full test suite (must pass before commits)
uv run pytest -n auto # Should show 75/75 tests passing
