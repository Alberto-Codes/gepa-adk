    ...
    for prof_file in PROF_DIR.glob("test_*.prof"):
        try:
            ...
            if total_time > SLOW_THRESHOLD:
                ...
                slow_tests.append(
                    {"name": test_name, "time": total_time, "file": prof_file.name}
                )
        except Exception:
            continue
                ...
                for i, line in enumerate(lines):
                    if func_pattern in line or (f"def {test_name}" in line):
                        ...
                        if "pytestmark" in content:
                            match = re.search(
                                r"pytestmark\s*=\s*pytest\.mark\.(\w+)", content
                            )
                            if match:
                                markers.append(match.group(1))
        ...
        if file_path:
            ...
            if not has_slow:
                ...
                has_slow_marker.append(
                    {"name": test_name, "time": test_time, "file": file_path}
                )
            print()
        else:
