...
def main() -> None:
    ...
    for result in results:
        ...
        if marks:
            stats["files_with_marks"] = stats["files_with_marks"] + 1  # type: ignore[assignment]
            for mark in marks:
                stats["mark_distribution"][mark] += 1
        else:
            stats["files_without_marks"] = stats["files_without_marks"] + 1  # type: ignore[assignment]
        ...
        if expected_type and expected_type not in marks:
            ...
            if not marks or not any(m in MARKERS for m in marks):
                missing_marks = stats["missing_marks"]
                if isinstance(missing_marks, defaultdict):
                    missing_marks[expected_type].append(str(relative_path))  # type: ignore[union-attr]
        ...
        if expected_type == "contract" and "contract" not in marks:
            if result["test_functions"] or result["test_classes"]:
                stats["inconsistent_marks"].append((str(relative_path), "contract"))  # type: ignore[union-attr]
        elif expected_type == "unit" and "unit" not in marks:
            if result["test_functions"] or result["test_classes"]:
                stats["inconsistent_marks"].append((str(relative_path), "unit"))  # type: ignore[union-attr]
        elif expected_type == "integration" and "integration" not in marks:
            if result["test_functions"] or result["test_classes"]:
                stats["inconsistent_marks"].append((str(relative_path), "integration"))  # type: ignore[union-attr]
    ...
    print("-" * 80)
    mark_dist = stats["mark_distribution"]
    if isinstance(mark_dist, defaultdict):
        for mark in MARKERS:
            count = mark_dist.get(mark, 0)
    ...
    missing_marks = stats["missing_marks"]
    if isinstance(missing_marks, defaultdict):
        for test_type, files in missing_marks.items():
            print(f"\n{test_type.upper()} tests missing @pytest.mark.{test_type}:")
            for file in files:  # type: ignore[union-attr]
                print(f"  - {file}")
    ...
    print("-" * 80)
    inconsistent_marks = stats["inconsistent_marks"]
    if isinstance(inconsistent_marks, list) and inconsistent_marks:
        for file, expected_mark in inconsistent_marks:
    ...
    for result in results:
        ...
        status = (
            "✓"
            if (expected_type and expected_type in marks)
            or (not expected_type and marks)
            else "⚠"
        )
