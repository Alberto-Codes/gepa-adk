        ...
        for file in files:
            if file.startswith("test_") and file.endswith(".py"):
                ...
                try:
                    ...
                    if test_name in content or test_name.replace("test_", "def test_") in content:
                        # Check for markers
                        markers = set()
                        lines = content.splitlines()
                        for i, line in enumerate(lines):
                            if test_name in line or test_name.replace("test_", "def test_") in line:
                                # Look backwards for markers
                                for j in range(max(0, i - 10), i):
                                    if "@pytest.mark." in lines[j]:
                                        marker = lines[j].split("@pytest.mark.")[1].split("(")[0].split()[0]
                                        markers.add(marker)
                        return markers
                except Exception:
                    continue
...
def main() -> None:
    ...
    prof_files = [f for f in prof_files if f.name != "combined.prof"]  # Exclude combined
    ...
    if (PROF_DIR / "combined.prof").exists():
        ...
        print(f"  Total function calls: {combined_stats.total_calls:,}")
        print(f"  Total time: {combined_stats.total_tt:.2f}s")
        print()
    ...
    for result in results:
        ...
        if "unit" in result.get("file", "").lower() or "/unit/" in result.get("file", ""):
            unit_times.append(time)
        elif "contract" in result.get("file", "").lower() or "/contract" in result.get("file", ""):
            contract_times.append(time)
        elif "integration" in result.get("file", "").lower() or "/integration/" in result.get("file", ""):
            integration_times.append(time)
    
    if unit_times:
        print(f"  Unit tests:      {len(unit_times):3} tests, avg {sum(unit_times)/len(unit_times):.3f}s, max {max(unit_times):.3f}s")
    if contract_times:
        print(f"  Contract tests:  {len(contract_times):3} tests, avg {sum(contract_times)/len(contract_times):.3f}s, max {max(contract_times):.3f}s")
    if integration_times:
        print(f"  Integration tests: {len(integration_times):3} tests, avg {sum(integration_times)/len(integration_times):.3f}s, max {max(integration_times):.3f}s")
    ...
    for test in very_slow[:10]:
        print(f"  - {test.get('test_name', 'unknown')}: {test.get('total_time', 0):.3f}s")
