...
evolution:minibatch
:   A fresh seeded sample of trainset rows that gates each mutation proposal
    before its full evaluation. The proposal is evaluated on the sampled rows
...
:   A JSON file holding the engine's own state, written atomically after the
    ...
    `resume=True` restores it instead of evaluating the baseline, continuing
    the iteration count, the stagnation counter and the evaluation counter
    without re-evaluating any candidate it already scored.

    **Usage:** Config fields (`EvolutionConfig.checkpoint_path`, `EvolutionConfig.resume`), log event (`checkpoint.resumed`).
