...
a doomed docs build. CI also checks the release PR lockfile in the planner:
a stale intermediate head fails the required gate before allocating compatibility
runners, while its lockfile follow-up still runs the full matrix. Normal ready PRs
still build docs with `uv sync --locked`;
