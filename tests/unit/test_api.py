...
class TestEvolveReflectionAgent:
    ...
    async def test_evolve_passes_reflection_agent_to_adapter(
        ...
        with (
            ...
            # Verify ADKAdapter was created with a proposer (constructed from reflection_agent)
            call_kwargs = mock_adapter_class.call_args[1]
            assert "proposer" in call_kwargs
            assert call_kwargs["proposer"] is not None
...
class TestEvolveDefaultReflectionBehavior:
    ...
    async def test_evolve_without_reflection_agent_uses_default(
        ...
        with (
            ...
            # Verify ADKAdapter was created with a proposer (from default reflection_agent)
            call_kwargs = mock_adapter_class.call_args[1]
            assert "proposer" in call_kwargs
            assert call_kwargs["proposer"] is not None
