...
class TestMultiAgentAdapterConstructor:
    ...
    def test_constructor_accepts_empty_component_list(
        ...
        agents = {
            "generator": LlmAgent(
                name="generator", model="gemini-3.6-flash", instruction="Generate"
            ),
            "validator": LlmAgent(
                name="validator", model="gemini-3.6-flash", instruction="Validate"
            ),
        }
        components = {
            "generator": ["instruction"],
...
class TestMultiAgentAdapterProposerDelegation:
    ...
    async def test_propose_new_texts_merges_partial_result(
        ...
        agents = {
            ...
            "critic": LlmAgent(
                name="critic", model="gemini-3.6-flash", instruction="Review"
            ),
        }
        scorer = MockScorer()
