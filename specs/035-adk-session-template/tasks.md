...
- [ ] T014 [US2] Verify REFLECTION_INSTRUCTION contains both `{component_text}` and `{trials}` placeholders in src/gepa_adk/engine/adk_reflection.py
- [ ] T015 [US2] Ensure session state dict includes both keys in src/gepa_adk/engine/adk_reflection.py
- [ ] T016 [US2] Add error handling for partial state scenarios in src/gepa_adk/engine/adk_reflection.py
...
- [ ] T017 [P] [US3] Update docs/guides/reflection-prompts.md with `{key}` template syntax section
- [ ] T018 [P] [US3] Add code examples showing session state setup and placeholder usage in docs/guides/reflection-prompts.md
- [ ] T019 [P] [US3] Document optional `{key?}` syntax for graceful missing key handling in docs/guides/reflection-prompts.md
- [ ] T020 [US3] Add migration notes from f-string workaround to template syntax in docs/guides/reflection-prompts.md
...
- [ ] T021 [P] Integration test with Gemini model in tests/integration/test_reflection_template.py
- [ ] T022 [P] Integration test with Ollama model in tests/integration/test_reflection_template.py (if available)

### Documentation Build Verification

- [ ] T023 Verify `uv run mkdocs build` passes without warnings
- [ ] T024 Preview docs with `uv run mkdocs serve` and verify reflection-prompts guide renders correctly

### Cross-Cutting Tasks

- [ ] T025 Run full test suite to verify no regressions (`uv run pytest`)
- [ ] T026 Verify output equivalence: template-based vs f-string workaround produce same results
- [ ] T027 Close GitHub Issue #99 with implementation summary
...
**Phase 5 (US3)**:
- T017, T018, T019 (docs) can run in parallel

**Phase 6 (Verification)**:
- T021 and T022 (integration tests) can run in parallel
