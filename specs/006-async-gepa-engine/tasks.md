...
- [ ] T011 [P] [US1] Unit test for baseline evaluation (max_iterations=0) in `tests/unit/engine/test_async_engine.py`
- [ ] T012 [P] [US1] Unit test for basic loop execution (max_iterations=5) in `tests/unit/engine/test_async_engine.py`
- [ ] T013 [P] [US1] Unit test for iteration history completeness in `tests/unit/engine/test_async_engine.py`

### Implementation for User Story 1

- [ ] T014 [US1] Implement `_initialize_baseline()` method for first evaluation in `src/gepa_adk/engine/async_engine.py`
- [ ] T015 [US1] Implement `_evaluate_candidate()` method with score aggregation (mean) in `src/gepa_adk/engine/async_engine.py`
- [ ] T016 [US1] Implement `_propose_mutation()` method calling adapter's reflective methods in `src/gepa_adk/engine/async_engine.py`
- [ ] T017 [US1] Implement `_record_iteration()` method for history tracking in `src/gepa_adk/engine/async_engine.py`
- [ ] T018 [US1] Implement `_build_result()` method to create frozen EvolutionResult in `src/gepa_adk/engine/async_engine.py`
- [ ] T019 [US1] Implement core `run()` method with basic loop (max_iterations only) in `src/gepa_adk/engine/async_engine.py`
- [ ] T020 [US1] Run User Story 1 tests to verify baseline-only and basic loop functionality

**Checkpoint**: User Story 1 complete - engine runs evolution loop and returns valid EvolutionResult

---

## Phase 4: User Story 2 - Early Stopping on Convergence (Priority: P2)
...
- [ ] T021 [P] [US2] Unit test for early stopping when patience exhausted in `tests/unit/engine/test_async_engine.py`
- [ ] T022 [P] [US2] Unit test for patience=0 disabling early stop in `tests/unit/engine/test_async_engine.py`
- [ ] T023 [P] [US2] Unit test for patience reset on improvement in `tests/unit/engine/test_async_engine.py`

### Implementation for User Story 2

- [ ] T024 [US2] Implement `_should_stop()` method with max_iterations and patience checks in `src/gepa_adk/engine/async_engine.py`
- [ ] T025 [US2] Add stagnation counter increment on rejection in `run()` method in `src/gepa_adk/engine/async_engine.py`
- [ ] T026 [US2] Add stagnation counter reset on acceptance in `run()` method in `src/gepa_adk/engine/async_engine.py`
- [ ] T027 [US2] Update `run()` to use `_should_stop()` for loop termination in `src/gepa_adk/engine/async_engine.py`
- [ ] T028 [US2] Run User Story 2 tests to verify early stopping behavior

**Checkpoint**: User Story 2 complete - engine stops early when converged

---

## Phase 5: User Story 3 - Accept Improved Candidates (Priority: P3)
...
- [ ] T029 [P] [US3] Unit test for accepting proposal above threshold in `tests/unit/engine/test_async_engine.py`
- [ ] T030 [P] [US3] Unit test for rejecting proposal below threshold in `tests/unit/engine/test_async_engine.py`
- [ ] T031 [P] [US3] Unit test for threshold=0.0 accepting any improvement in `tests/unit/engine/test_async_engine.py`
- [ ] T032 [P] [US3] Unit test for candidate lineage tracking (generation, parent_id) in `tests/unit/engine/test_async_engine.py`

### Implementation for User Story 3

- [ ] T033 [US3] Implement `_should_accept()` method with threshold comparison in `src/gepa_adk/engine/async_engine.py`
- [ ] T034 [US3] Implement `_accept_proposal()` method with lineage update in `src/gepa_adk/engine/async_engine.py`
- [ ] T035 [US3] Update `run()` to use `_should_accept()` and `_accept_proposal()` in `src/gepa_adk/engine/async_engine.py`
- [ ] T036 [US3] Run User Story 3 tests to verify acceptance logic and lineage tracking
...
- [ ] T037 [P] Add Google-style docstrings to all public methods in `src/gepa_adk/engine/async_engine.py`
- [ ] T038 [P] Add Google-style docstrings to `_EngineState` class in `src/gepa_adk/engine/async_engine.py`
- [ ] T039 [P] Update `src/gepa_adk/__init__.py` to export engine module
- [ ] T040 [P] Run `uv run ruff check --fix` and `uv run ruff format` on engine module
- [ ] T041 [P] Run `uv run ty check` to verify type annotations
- [ ] T042 Run full test suite `uv run pytest -n auto` to verify no regressions
- [ ] T043 Validate quickstart.md examples work with implemented engine
- [ ] T044 Run `uv run pytest --cov=src/gepa_adk/engine --cov-report=term-missing` for coverage report
...
| US1 (P1) | 11 | 4 parallel (tests) |
| US2 (P2) | 8 | 3 parallel (tests) |
| US3 (P3) | 8 | 4 parallel (tests) |
| Polish | 8 | 5 parallel |
| **Total** | **44** | |
